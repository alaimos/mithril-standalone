 ...
 * @since 08/01/2016
 */
public class NeighborhoodsWriter extends AbstractDataWriter<VisitTree> {
    ...
    public DataWriterInterface<VisitTree> write(VisitTree data) {
        try (PrintStream ps = new PrintStream(getOutputStream(append))) {
            if (!append) {
                ps.println("#Root\tNeighbors\tAccumulator\tp-value");
            }
            String neighbors = data.children().stream().map(VisitTree::getObject).map(GraphVisitNode::getNode)
                                   .map(NodeInterface::getId)
                                   .collect(Collectors.joining(","));
            writeDelimited(ps, "\t", data.getObject().getNode().getId(),
                    neighbors, data.getNeighborhoodAccumulator(), data.getNeighborhoodPValue());
            ps.println();
        }
        return this;
    }
