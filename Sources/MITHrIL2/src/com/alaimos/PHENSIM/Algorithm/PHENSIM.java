 ...
 * <li>Optional parameter <code>fast</code> of type <code>Boolean</code></li>
 * </ul>
 *
...
public class PHENSIM extends AbstractAlgorithm<PhensimRun> {
    ...
    public void run() {
        ...
        fast = getOptionalParameter("fast", Boolean.class).orElse(false);
        ...
        if (fast) {
            var builders = new HashMap<PathwayInterface, WeightMatrixBuilder>();
            for (var p : repository) {
                if (p.hasGraph() && p.getGraph().countNodes() > 0 && p.getGraph().countEdges() > 0) {
                    var b = WeightMatrixBuilder.getInstance(p, threads, s -> notifyObservers("phensimMessage", s));
                    builders.put(p, b);
                    b.setNonExpressed(nonExpressedNodes);
                }
            }
            output.setBuilders(builders);
        } else {
        ...
        for (var i = 0; i < n; i++) {
            ...
            executor.submit(() -> {
                try {
                    ...
                    if (fast) {
                        new FasterSimulationStep(generator, epsilon, results,
