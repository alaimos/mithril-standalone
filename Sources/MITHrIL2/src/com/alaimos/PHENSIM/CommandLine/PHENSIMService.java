...
import com.alaimos.PHENSIM.Algorithm.PathwayMerger;
...
import com.alaimos.PHENSIM.Data.Reader.PHENSIMInputReader;
import com.alaimos.PHENSIM.Data.Writer.ExtendedSIFWriter;
import com.alaimos.PHENSIM.Data.Writer.PhensimMatrixWriter;
import com.alaimos.PHENSIM.Data.Writer.PhensimRunWriter;
import com.alaimos.PHENSIM.Data.Writer.SBMLWriter;
...
public class PHENSIMService implements Service {
    ...
    protected RepositoryInterface getPathwayRepository(Species s) {
        return getEnrichedRepository(s, options.randomSeed, this::report, options.enrichers,
                options.enrichmentEvidenceType, options.enrichersParameters, options.reactome);
    }
    ...
    public void run() {
        ...
        try {
            ...
            com.alaimos.MITHrIL.Algorithm.PathwayMerger pm;
            if (options.reactome) {
                pm = new PathwayMerger();
            } else {
                pm = new com.alaimos.MITHrIL.Algorithm.PathwayMerger();
            }
            pm.init()
            ...
            var constraints = new PHENSIMInputReader().setFile(options.input).read();
            ...
            simp.init()
                    ...
                    .setParameter("fast", options.fast)
            ...
            if (options.outputSBML != null) {
                report("Writing SBML output");
                new SBMLWriter(r).write(options.outputSBML, output);
                reportln("...OK!");
            }
            if (options.outputSIF != null) {
                report("Writing Extended SIF output");
                new ExtendedSIFWriter(r).write(options.outputSIF, output);
                reportln("...OK!");
            }
        } catch (Exception e) {
            System.out.println("\n" + e.getMessage());
